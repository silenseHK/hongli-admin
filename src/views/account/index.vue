  ...
  methods: {
    ...
    submitBinding(){
      ...
      canClickFn(() => {
        ...
        bindingAccount(params).then(resp => {
          if(resp.code == 200){
            this.$message({
              message: '绑定成功',
              type: 'success'
    ...
    searchId(){
      ...
      canClickFn(() => {
        getAgencyStaffInfo({phone: this.bindingInfo.searchTel}).then(resp => {
         if(resp.code == 200){
            this.showInps = true
    ...
    getSystems() {
      ...
      getSystem(pageSize, pageIndex).then((res) => {
        if (res.code === 200) {
    ...
    handelSearch() {
      const { name, num } = this.accountData;
      const params = {
        phone: name,
        nickname: num,
        limit: this.pageSize,
    ...
    handelSubmit() {
      this.$refs.informRef.validate((valid) => {
        if (valid) {
          ...
          if (this.status === 1) {
            addAgencyStaff(params).then((res) => {
              if (res.code === 200) {
            ...
            params.id = id
            editAgencyStaff(params).then((res) => {
              if (res.code === 200) {
                this.pageIndex = 1
